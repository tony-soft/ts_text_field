...
class TsEditableTextState extends State<TsEditableText>
    with AutomaticKeepAliveClientMixin<TsEditableText>, WidgetsBindingObserver, TickerProviderStateMixin<TsEditableText>, TextSelectionDelegate, TextEditingActionTarget
  ...
  RevealedOffset _getOffsetToRevealCaret(Rect rect) {
    ...
    if (!_isMultiline) {
      ...
      additionalOffset = expandedRect.height >= editableSize.height ? editableSize.height / 2 - expandedRect.center.dy : 0.0.clamp(expandedRect.bottom - editableSize.height, expandedRect.top);
  ...
  void _restartConnectionIfNeeded() {
    ...
    final TextInputConnection newConnection = currentAutofillScope?.attach(this, textInputConfiguration) ?? TextInput.attach(this, _effectiveAutofillClient.textInputConfiguration);
  ...
  void _handleSelectionChanged(TextSelection selection, SelectionChangedCause? cause) {
    ...
    if (cause != SelectionChangedCause.keyboard) requestKeyboard();
  ...
  void _cursorWaitForStart(Timer timer) {
    ...
    assert(!TsEditableText.debugDeterministicCursor);
    _cursorTimer?.cancel();
  ...
  void _startCursorTimer() {
    ...
    if (TsEditableText.debugDeterministicCursor) return;
    if (widget.cursorOpacityAnimates) {
  ...
  void _updateCaretRectIfNeeded() {
    if (_hasInputConnection) {
      if (renderEditable.selection != null && renderEditable.selection!.isValid && renderEditable.selection!.isCollapsed) {
        final TextPosition currentTextPosition = TextPosition(offset: renderEditable.selection!.baseOffset);
  ...
  VoidCallback? _semanticsOnCut(TextSelectionControls? controls) {
    return widget.selectionEnabled && cutEnabled && _hasFocus && controls?.canCut(this) == true ? () => controls!.handleCut(this, _clipboardStatus) : null;
  }
          ...
          return CompositedTransformTarget(
            ...
            child: Semantics(
              ...
              child: _Editable(
                ...
                showCursor: TsEditableText.debugDeterministicCursor ? ValueNotifier<bool>(widget.showCursor) : _cursorVisibilityNotifier,
